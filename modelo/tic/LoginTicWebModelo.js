...
class LoginTicWebModelo {
  constructor() {
    if (LoginTicWebModelo.instance) {
      return LoginTicWebModelo.instance;
    }
    LoginTicWebModelo.instance = this;
  }

  async buscarUsuarioPorDocumento(documento) {
    const query = 'SELECT idFuncionario, documento, nombres, rol, llave, estado, intentosFallidos FROM funcionarios WHERE documento = ? AND rol = "MesaAyuda"';
    try {
      const result = await servicio.query(query, [documento]);
      return result.length ? result[0] : null;
    } catch (err) {
      console.error("Error en Modelo (buscarUsuarioPorDocumento):", err);
      throw new Error("Error interno al buscar el Funcionario.");
    }
  }
  ...
  async guardarTokenSesion({ documento, nombres, token, dispositivo = "Web" }) {
    ...
    try {
      await servicio.query(query, [documento, nombres, "MesaAyuda", token, dispositivo]);
    } catch (err) {
...
module.exports = new LoginTicWebModelo();