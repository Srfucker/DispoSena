...
class LoginVigilanteWebModelo {
    constructor() {
        if (LoginVigilanteWebModelo.instance) {
            return LoginVigilanteWebModelo.instance;
        }
        LoginVigilanteWebModelo.instance = this;
    }

    async buscarUsuarioPorDocumento(documento) {
        const query = 'SELECT idFuncionario, documento, nombres, rol, llave, estado, intentosFallidos FROM funcionarios WHERE documento = ? AND rol = "Vigilante"';
        try {
            const result = await servicio.query(query, [documento]);
            return result.length ? result[0] : null;
        } catch (err) {
            console.error("Error en Modelo (buscarUsuarioPorDocumento):", err);
            throw new Error("Error interno al buscar el Vigilante.");
        }
    }
    ...
    async guardarTokenSesion({ documento, nombres, token, dispositivo = "Web" }) {
        ...
        try {
            await servicio.query(query, [documento, nombres, "Vigilante", token, dispositivo]);
        } catch (err) {
...
module.exports = new LoginVigilanteWebModelo();