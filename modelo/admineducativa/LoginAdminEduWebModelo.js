...
class LoginAdminEduWebModelo {
    constructor() {
        if (LoginAdminEduWebModelo.instance) {
            return LoginAdminEduWebModelo.instance;
        }
        LoginAdminEduWebModelo.instance = this;
    }

    async buscarUsuarioPorDocumento(documento) {
        const query = 'SELECT idFuncionario, documento, nombres, rol, llave, estado, intentosFallidos FROM funcionarios WHERE documento = ? AND rol = "AdminEducativo"';
        try {
            const result = await servicio.query(query, [documento]);
            return result.length ? result[0] : null;
        } catch (err) {
            console.error("Error en Modelo (buscarUsuarioPorDocumento):", err);
            throw new Error("Error interno al buscar el Administrador Educativo.");
        }
    }
    ...
    async guardarTokenSesion({ documento, nombres, token, dispositivo = "Web" }) {
        const query = 'INSERT INTO tokenfuncionario (usuarioFuncionario, nombresFuncionario, rol, token, dispositivo) VALUES (?, ?, ?, ?, ?)';
        try {
            await servicio.query(query, [documento, nombres, "Administrador Educativo", token, dispositivo]);
        } catch (err) {
    ...
    async buscarTokenSesionActiva(documento, dispositivo = "Web") {
...
module.exports = new LoginAdminEduWebModelo();