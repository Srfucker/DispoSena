const VigilanteModelo = require('../../modelo/coordinador/VigilanteModelo');
const Validador = require('../Validador');


class VigilanteControlador {
    constructor() {
    ...
    async crearVigilante(req, res) {
        ...
        try {
            ...
            const nuevoFuncionario = new VigilanteModelo({
                tipoDoc: tipoDoc,
                documento: documento,
                nombres: nombres,
                telefono: telefono,
            ...
            return res.status(201).json({
                mensaje: 'Vigilante creado con éxito',
            ...
            if (err.message && err.message.includes('Campo Duplicado: ')) {
                return res.status(409).json({ // 409 Conflict
                    error: 'Ya existe un Vigilante con estos datos. Por favor, verifique el tipo de documento y el número de documento.'
                });
            }
            // Manejo de otros errores internos del servidor
            console.error('Error en VigilanteControlador.crearAdmin:', err);
            return res.status(500).json({ error: 'Error interno del servidor: ' + err.message });
        }
    }
}

module.exports = VigilanteControlador;